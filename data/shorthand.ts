...
export const prot = (num: number, subs = true) => doProtections(num, true, subs);
export const botProt = (num: number, subs = true) => doProtections(num, false, subs);
...
export const tradePick = (year: number, round: number, team: TeamAbbr): PickMeta => ({
  id: `${year}.${round}` as Id,
  details: `To ${team}`,
  summary: {
    isOwn: true,
...
export const getPick = (year: number, round: number, team: TeamAbbr): PickMeta => ({
