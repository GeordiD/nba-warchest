  ...
  Object.keys(picksByYear).forEach(year => {
    picksByYear[year].forEach(async pick => {
      ...
      const pickRequest = {
        ...
        round: round,
        originator: getTeamId(pick.originator, pick),
        isForfeited: pick.isForfeited,
      }

      if (pick.position) pickRequest.position = pick.position;
      if (pick.owner) pickRequest.toTeam = getTeamId(pick.owner, pick);

      const pickReq = await pb.collection('picks').create(pickRequest, {
        requestKey: `${year}-${round}-${pick.originator}`,
      })
      ...
      if (pick.protections) {
        const promises = pick.protections.groups.map(async protection => {
          return await pb.collection('protections').create({
            ...
            pick: pickReq.id,
          }, { requestKey: `${year}-${round}-${pickReq.id}-protections-${protection.owner}`})
        });

        const results = await Promise.all(promises);

        await pb.collection('picks').update(pickReq.id, {
          protections: results.map(protectionRes => protectionRes.id),
        }, { requestKey: `${year}-${round}-${pickReq.id}-protections-update` });
      }
    })
  })

  console.log(deepLog(picksByYear));
}
